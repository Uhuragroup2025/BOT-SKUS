...
with open('app/api/generate/route.ts', 'r') as f:
    text = f.read()
...
start_marker = r'        let content = "";\n        let imageContent = "";'
end_marker = r'        parsedContent.packaging_analysis = parsedImageContent.packaging_analysis || null;'

replacement = """        let content = "";
        ...
        // 3.1 Start SEO generation in parallel
        ...
        // 3.2 Sequential Image Prompt Pipeline
        try {
            ...
            if (packaging_analysis) {
                ...
                visualAssets = await Promise.all(momentPromises);
            } else {
        ...
        try {
            ...
            console.error("----- RAW AI SEO OUTPUT THAT FAILED TO PARSE -----");
...
with open('app/api/generate/route.ts', 'w') as f:
    f.write(text)
