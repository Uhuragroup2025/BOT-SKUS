...
# Google Antigravity · FAL.ai
# Versión 5.0 — SEQUENTIAL + PACKAGING-DRIVEN + COMPOSITE LIFESTYLE
# ============================================================
...
## ENDPOINTS FAL.AI DE REFERENCIA

```
flux/fill (Flujo A — background replace):
  https://fal.run/fal-ai/flux-pro/v1/fill
  Params requeridos: image_url, mask_url, prompt, strength
  Strength range: 0.40–0.55

flux/ultra (Flujo B paso 1 — generación de escena):
  https://fal.run/fal-ai/flux-pro/v1.1-ultra
  Params requeridos: prompt, aspect_ratio
  No necesita image_url en paso 1 (genera escena sin producto)

recraft v3 (armonización opcional):
  https://fal.run/fal-ai/recraft-v3
  Params requeridos: image_url, prompt, strength
  Strength: 0.20–0.30 máximo
```
"""
...
# I also need to export two new helper functions instead of `constructImagePrompt`. The SEO/GEO stuff remains untouched.
new_functions = """export function constructImageAnalysisPrompt() {
  return `
